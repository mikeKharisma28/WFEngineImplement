...
namespace WorkflowApi.Controllers
{
    ...
    public class WorkflowController : ControllerBase
    {
        ...
        public async Task<IActionResult> Schemes()
        {
            ...
            var list = await WorkflowInit.Provider.WorkflowScheme.SelectAllWorkflowSchemesWithPagingAsync(connection, orderParameters, paging);
        ...
        public async Task<IActionResult> Instances()
        {
            ...
            // getting schemes from the database
            var processes = await WorkflowInit.Provider.WorkflowProcessInstance.SelectAllWithPagingAsync(connection, orderParameters, paging);
        ...
        public async Task<IActionResult> CreateInstance(string schemeCode)
        {
            ...
            var processInstanceEntity = await WorkflowInit.Provider.WorkflowProcessInstance.SelectByKeyAsync(connection, processId);

            // converting instances to DTO
        ...
        public async Task<IActionResult> ExecuteCommand(Guid processId, string command, string identityId)
        {
            // getting available commands for a process instance
            var commands = await WorkflowInit.Runtime.GetAvailableCommandsAsync(processId, identityId);
            // searching for the necessary command
            var workflowCommand = commands?.First(c => c.CommandName == command) ?? throw new ArgumentException($"Command {command} not found");
        ...
        /// <param name="schemeCode">Scheme codde</param>
